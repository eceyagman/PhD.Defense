...
\textbf{Beyond cognitive abilities: socioemotional skills (SES)}

\vspace{0.4cm}
\begin{itemize}
    \item Regulate emotions, maintain relationships, manage goals and decisions
    \item Perseverance, motivation, teamwork \citep{Kautz2014-tm, Deming2017-ck}
    \item Predict educational attainment, earnings, well-being \citep{Heckman2012-nt}
    \item Malleable: can be developed through instruction
\end{itemize}

\vspace{0.5cm}
\textbf{Schools are expected to foster SES, but lack the infrastructure to track them}
\end{frame}
...
\vspace{0.5cm}
\emph{Pentabilities}: 35 observable behaviors $\times$ 5 domains $\times$ repeated 360$^{\circ}$ assessment (teacher, peer, self) via digital platform
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.65\textwidth}
    ...
    \begin{itemize}
      \item Curriculum-embedded (not a separate class)
...
\midrule
Responsibility & Perseveres, works consistently, respects rules \\
Autonomy & Brings up ideas, takes initiative \\
Cooperation & Listens, encourages participation, resolves conflicts \\
Emotional Mngt & Stays calm under pressure, accepts mistakes \\
Thinking & Asks good questions, proposes strategies \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.3cm}
Each behavior scored 1--5 by multiple raters
\end{frame}
...
\vspace{0.1cm}
Treatment: 69 classrooms (1,164 students) \quad Control: 72 classrooms (1,200 students)
...
\begin{tabular}{lllll}
\toprule
Instrument & Setting & Raters & T/C & Chapters \\
\midrule
\textbf{CO} & Regular class, repeated & Self, Peer, Tch, EO & T only & 1, 2 \\
\textbf{FA} & Lab-in-the-field, random groups & Self, Peer, EO & T + C & 2, 3 \\
\textbf{BESSI} & Endline survey & Self, Peer, Tch & T + C & 2, 3 \\
\bottomrule
\end{tabular}
\end{frame}
...
\textbf{For each student, four perspectives:}
\begin{itemize}
    \item \textbf{Self} \quad $\cdot$ \quad \textbf{Peers} \quad $\cdot$ \quad \textbf{Teacher} \quad $\cdot$ \quad \textbf{External observer}
\end{itemize}

\vspace{0.5cm}
$\Rightarrow$ \textbf{This lets us:}
\begin{itemize}
    \item Benchmark self-perception against ability proxies
    \item Decompose evaluation gaps by student gender $\times$ peer gender
